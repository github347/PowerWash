...
class CustomUserCreationForm(UserCreationForm):
    ...
    username = forms.CharField(max_length=150, required=True, help_text='Username must be unique.')
    ...
    password1 = forms.CharField(
        ...
        max_length=8, 
        required=True,
        help_text='Password must be between 6 and 8 characters, including at least one uppercase letter and one special character.'
    )

    # Make password2 a raw CharField (no validation applied)
    password2 = forms.CharField(
        widget=forms.PasswordInput,
        min_length=6, 
        max_length=8, 
        required=False,  # Optional to prevent Django from enforcing validation
